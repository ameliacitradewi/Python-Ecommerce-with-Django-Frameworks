    ...
    if request.user.is_authenticated:
        ...
        if order_query.exists():
            ...
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                ...
                pesan = f"Your shopping cart has been updated! You have { order_produk_item.quantity } items on your cart now."
                messages.info(request, pesan)
                return redirect('toko:produk-detail', slug = slug)
            else:
                order.produk_items.add(order_produk_item)
                messages.info(request, 'Great choice! Your product has been added to the shopping cart!')
                return redirect('toko:produk-detail', slug = slug)
        else:
        ...
        if order_query.exists():
            ...
            if order.produk_items.filter(produk_item__slug=produk_item.slug).exists():
                try: 
                    ...
                    pesan = f"Oops! Your product has been deleted. Are you sure?"
                    messages.info(request, pesan)
                    return redirect('toko:produk-detail',slug = slug)
                except ObjectDoesNotExist:
                    print('Error: invalid product.')
            else:
                messages.info(request, 'There is no such product on your cart.')
                return redirect('toko:produk-detail',slug = slug)
        else:
...
class OrderSummaryView(LoginRequiredMixin, generic.TemplateView):
    def get(self, *args, **kwargs):
        try:
            ...
            messages.error(self.request, 'You have 0 ongoing order.')
            return redirect('/')
...
def paypal_return(request):
    if request.user.is_authenticated:
        try:
            ...
            messages.info(request, 'Payment is confirmed. Thank you.')
            return redirect('toko:home')
        except ObjectDoesNotExist:
            messages.error(request, 'Please recheck your order.')
            return redirect('toko:order-summary')
    else:
        return redirect('/account/login')
...
def paypal_cancel(request):
    messages.error(request, 'Oops! Your payment is cancelled. Try again?')
    return redirect('toko:order-summary')
